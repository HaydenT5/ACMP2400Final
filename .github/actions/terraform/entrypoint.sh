...
if [[ "$TF_STAGE" == "stage1" ]]; then
  ...
  terraform -chdir=${INPUT_TF_STATE} init -backend-config="key=${INPUT_STATE_KEY}.tfstate"
  terraform -chdir=${INPUT_TF_STAGE} apply -auto-approve -var="ARM_CLIENT_ID=${INPUT_ARM_CLIENT_ID}" -var="ARM_CLIENT_SECRET_=${INPUT_ARM_CLIENT_SECRET}" -var="DJANGO_SECRET_KEY_PROD=${INPUT_DJANGO_SECRET_KEY_PROD}"
fi      
